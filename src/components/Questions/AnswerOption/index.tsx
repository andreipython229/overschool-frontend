import { FC, memo, ReactNode, useState, ChangeEvent, PointerEvent } from 'react'
...
export const AnswerOption: FC<AnswerOptionT> = memo(({ children, id, answer }) => {
  ...
  const onAnswerPointerDown = (event: PointerEvent<SVGSVGElement | SVGPathElement>) => {
    ...
    >
      <div className={styles.answerOptionsBlock}>
        <div className={styles.answerOptionsBlock_inputWrapper}>
          {children}
          <InputBlock name={''} type={'text'} value={answerText} placeholder={'Вариант ответа'} onChange={handleChangeAnswer} />
          ...
          <div className={styles.answerOptionsBlock_inputWrapper_grab}>
            <IconSvg width={21} height={14} viewBoxSize="0 0 21 14" path={grabIconPath} onPointerDown={onAnswerPointerDown} />
          </div>
        </div>
      </div>
    </Reorder.Item>
