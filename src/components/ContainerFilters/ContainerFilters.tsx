...
export const ContainerFilters: FC<ContainerFiltersT> = memo(({ title, ...props }: ContainerFiltersT) => {
  const { courses } = useAppSelector(allCoursesSelector)

  if (title && title === 'курсы') {
    return (
      <div>
        <Filter name={'course'} header={'ВЫБЕРИТЕ КУРСЫ'} data={courses.map((course: CoursesT) => course.name)} />
      </div>
    )
