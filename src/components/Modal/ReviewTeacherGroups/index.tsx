...
export const ReviewTeacherGroups: FC<reviewTeacherGroupsT> = ({ closeModal, name, email, id }) => {
  ...
  useEffect(() => {
    if (isSuccess && courses) {
      ...
      const teacherCourses = updatedCourses
        .filter(course => course.student_groups.length !== 0)
        .map(course => ({
          ...course,
    ...
    <div /*onSubmit={handleCreatEmployee}*/ className={styles.main_employee}>
      ...
      <div className={styles.main_employee_container}>
        ...
        <div className={styles.main_employee_course}>
          {groups?.length !== 0 ? (
            <>
              <span className={styles.main_employee_course_title} style={{ marginBottom: 12 }}>
                Группы ментора:
              </span>
              {groups?.map(({ course_id, course_name, name, available_teachers, new_teacher, changeType, changeTeacher }: teacherGroupT) => (
                <>
                    ...
                    <div className={styles.main_employee_change}>
                      ...
                      {changeTeacher && (
                        <Dropdown
                          ...
                          onChange={e => handleChangeTeacher(course_id, e)}
