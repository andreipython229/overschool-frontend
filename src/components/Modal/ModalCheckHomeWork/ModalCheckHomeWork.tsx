...
export const ModalCheckHomeWork: FC<modalHomeworkT> = ({ userHomeworkId }) => {
  const [isOpen, setIsOpen] = useState<boolean>(false)

  const { data: homeworkData } = useFetchUserHomeworkQuery(userHomeworkId)

  const handleToggleHiddenBlocks = (): void => {
    setIsOpen(!isOpen)
  }

  const [mmddyyyy, hoursAndMinutes] = convertDate(new Date(homeworkData?.updated_at))
    ...
    <div className={styles.modal_container}>
      <div style={{ boxSizing: 'border-box' }} className={styles.modal_content}>
        ...
        <div className={styles.modal_visibility_block}>
          ...
          <div className={styles.speech_bubble}>{homeworkData?.teacher_message}</div>
