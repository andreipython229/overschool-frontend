...
export const BasicSettings: FC<BasicSettingsT> = ({
  ...
  const handleSendEmail = async () => {
    if (email) {
      try {
        ...
        setEmailsWithAccess(prevEmails => [...new Set([...prevEmails, ...newEmails])])
      } catch (error) {
  ...
  const handleEmailSelectionChange = (email: string) => {
    setSelectedEmails(prevSelected => (prevSelected.includes(email) ? prevSelected.filter(e => e !== email) : [...prevSelected, email]))
  }
  ...
  return (
    <div className={`${styles.basic_settings}`}>
      ...
      <div className={styles.btn}>
        <Button style={{ padding: '15px 89px' }} onClick={handleSendEmail} text={'Поделиться'} variant={'newPrimary'} />
        <Button style={{ padding: '15px 40px' }} variant={'cancel'} onClick={() => setRevokeAccessOpen(true)} text={'Отозвать доступ'} />
      </div>
      <ThemeProvider theme={theme}>
        <Dialog
          open={revokeAccessOpen}
          onClose={handleCloseRevokeAccess}
      ...
      <div className={styles.publish_switch}>
        ...
        <span className={styles.publish_switch_desc} style={{ margin: '20px 0' }}>
          курс попадает в каталог и становится доступным для общего поиска в интернете
        </span>
        <div className={styles.publish_switch_wrapper_switch}>
          {courseFind.baselessons_count && courseFind.baselessons_count >= 5 && toggleCheckbox ? (
      ...
      <div className={styles.publish_switch} style={{ marginTop: '20px' }}>
        ...
        <span className={styles.publish_switch_desc} style={{ margin: '20px 0' }}>
          курс можно найти по прямой ссылке даже при его отсутствии в каталоге
        </span>
        <div className={styles.publish_switch_wrapper_switch}>
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          <DialogContent>
            ...
            >
              Этот курс будет перемещен в корзину и останется там на протяжении недели. Вы сможете его восстановить в течение этого времени. Если вы
              не восстановите курс, он будет удален навсегда. Если вы уверены, что хотите удалить курс {`"${courseFind.name}"`}, нажмите {'удалить'}.
