...
export const BlocksController: FC<BlocksControllerT> = ({ openModal }) => {
  ...
  const setBlockVisibility = (id: number, isVisible: boolean) => {
    const newBlocks = {
      ...landing,
      ...Object.fromEntries(
        Object.entries(landing).map(([key, block]) => (block.id === id ? [key, { ...block, visible: isVisible }] : [key, block])),
      ),
    }
  ...
  return (
    <div className={styles.blocksController}>
      <div className={styles.blocksController_controls}>
        <Button variant="newSecondary" onClick={resetBlocks} text="По умолчанию" />
        <Button variant="newPrimary" onClick={sendlandingInfo} text="Сохранить изменения" />
      </div>
          ...
          <div key={block.id} className={styles.blocksController_wrapper}>
            <div className={styles.blocksController_wrapper_block}>
              ...
              <div className={styles.blocksController_wrapper_block_buttonPanel}>
                {!block.onlyShow && (
                  <button onClick={() => setBlockVisibility(block.id, false)}>
                    <VisibilityOffOutlinedIcon fontSize="large" />
                  </button>
                )}
                {block.canUp && (
                  <button onClick={() => moveUp(block.id)}>
                    <KeyboardArrowUp fontSize="large" />
                  </button>
                )}
                {block.canDown && (
                  <button onClick={() => moveDown(block.id)}>
