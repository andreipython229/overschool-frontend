...
import styles from './SalaryBlockCard.module.scss'
import { ChangeEvent, useState } from 'react'
import { changeBlocks, addFile } from 'store/redux/landing/constructorSlice'
import { TextareaAutosize } from '@mui/material'
import { DeleteForever } from '@mui/icons-material'

export const SalaryBlockCard: React.FC<CardPropsT> = ({ position, onDelete, isFirst }) => {
  const dispatch = useAppDispatch()
  const landing = useAppSelector(state => state.landing.blocks)

  const [titleValue, setTitleValue] = useState<string>(landing.income.chips[position].title)
  const [descriptionValue, setDescriptionValue] = useState<string>(landing.income.chips[position].description)

  const [cardImage, setCardImage] = useState<string>(landing.income.chips[position].photo)
    ...
    fileInput.onchange = event => {
      ...
      if (files) {
        if (files[0].size <= 7 * 1024 * 1024) {
          ...
          dispatch(
            addFile({
              key: `photo_income_${position}`,
              file: files[0],
            }),
          )

          // сохраняем временнную ссылку на файл в redux
          const chips = landing.income.chips.map(item => {
