...
import styles from './styles/AdvantageBlock.module.scss'
import { Add } from '@mui/icons-material'
import { AdvantageBlockCard } from './components/AdvantageBlockCard'
...
export const AdvantageBlock = () => {
  ...
  const deleteCard = (position: number) => {
    ...
    dispatch(removeFile({ key: `photo_advantage_${position}` }))

    const newChips = landing.advantage.chips
  ...
  const canAddButtonAfterCard = (position: number) => {
    ...
    return position + 1 === landing.advantage.chips.length || (position + 1) % 3 === 0
  }
        ...
        {getChips().map((card, index) => (
          <React.Fragment key={key - card.position}>
            <div className={styles.wrapper_cardContainer_chip}>
              ...
              <AdvantageBlockCard position={card.position} onDelete={() => deleteCard(card.position)} />
