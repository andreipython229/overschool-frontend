...
export const LinkBlockRedactor: React.FC = () => {
  ...
  useEffect(() => {
    if (linkButton) {
      ...
      if (linkButton.name !== updatedLinkButton.name || linkButton.link !== updatedLinkButton.link || linkButton.color !== updatedLinkButton.color) {
        dispatch(
          changeBlocks({
            ...blocks,
    ...
    <div className={styles.wrapper}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', alignItems: 'flex-start', gap: '20px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', alignItems: 'flex-start', gap: '10px' }}>
          ...
          <TextField value={buttonName} onChange={e => setButtonName(e.target.value)} required type="text" placeholder="Введите текст" fullWidth />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', alignItems: 'flex-start', gap: '10px' }}>
          <p className={styles.text}>Ссылка:</p>
          <TextField value={buttonLink} onChange={e => setButtonLink(e.target.value)} required type="text" placeholder="Вставьте ссылку" fullWidth />
        </div>
      </div>
    </div>
  )
