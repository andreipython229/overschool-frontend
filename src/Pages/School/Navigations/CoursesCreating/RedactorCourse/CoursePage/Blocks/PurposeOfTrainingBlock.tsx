...
export const PurposeOfTrainingBlock = () => {
  ...
  return (
    <div className={styles.wrapper}>
      <h2>Твой результа после курса</h2>
        ...
        {getChips().map((card, index) => (
          <React.Fragment key={key - card.position}>
            ...
            >
              ...
              <PurposeOfTrainingBlockCard position={card.position} onDelete={() => deleteCard(card.position)} />
      ...
      {landing.trainingPurpose.chips.length === 8 && (
        <div className={styles.wrapper_arrow}>
          <img src={isMobile ? arrowImageMobile : arrowImage} alt="arrowImage" />
        </div>
      )}
    </div>
  )
