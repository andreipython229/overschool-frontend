  ...
  return (
    <>
      ...
      <div className={styles.rating_param}>
        <label className={styles.rating_param_label} htmlFor="flag">
          Критерий рейтинга:
        </label>
        <br />
        ...
        {ratingData?.results.length ? (
          <>
            <thead className={styles.table_head}>
              <tr>
                <th style={{ width: '25%' }}>
                  <span style={{ margin: '0 7px 0 34px' }}>Ранг</span>
                </th>
                <th style={{ width: '50%' }}>
                  <span style={{ margin: '0 7px 0 0' }}>Фамилия и имя</span>
                </th>
                <th>
            ...
            <tbody className={styles.table_body}>
              {ratingData?.results.map((rating: studentRatingT, index) => (
                <tr key={rating.id} role="row" style={userId === rating.id ? { backgroundColor: 'rgba(211,175,207,0.53)' } : {}}>
                  ...
                  <td style={{ width: '50%' }}>
                    <span style={{ color: '#424345' }}>
                      {!rating.last_name && !rating.first_name ? 'Без имени' : `${rating.last_name} ${rating.first_name}`}
                    </span>
                  </td>
                  <td>
