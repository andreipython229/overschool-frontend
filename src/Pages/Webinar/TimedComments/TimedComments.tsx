...
export const TimedComments: React.FC<TimedCommentsProps> = ({
  currentTime,
  predefinedComments,
  userComments,
}) => {
  ...
  const visibleComments = useMemo(() => {
    return [...predefinedComments, ...userComments]
      .filter(comment => comment.time <= currentTime)
      ...
      {visibleComments.map((comment) => (
        <div key={comment.id} className={styles.timedComments_comment}>
          <div className={styles.timedComments_comment_header}>
            ...
            <span className={styles.timedComments_comment_header_role}>
              {comment.role}
            </span>
          </div>
