  ...
  return (
    <div className={styles.container}>
      <div className={styles.container_player}>
        {status !== 'live' ? (
          <div className={styles.container_player_message}>{message || 'Эфир скоро начнётся...'}</div>
        ) : videoId ? (
          <LiveVideoPlayer videoId={videoId} onReady={setPlayer} seekToSeconds={videoOffsetSeconds} />
        ) : uploadedVideoUrl ? (
          <InternalVideoPlayer uploadedVideoUrl={uploadedVideoUrl} seekToSeconds={videoOffsetSeconds} onReady={setPlayer} />
        ) : null}
      </div>

      {status === 'live' && (
        <div className={styles.container_comments}>
          <TimedComments currentTime={currentTime} predefinedComments={predefinedComments} userComments={userComments} />
