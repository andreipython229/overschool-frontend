    ...
    <>
        ...
        {data?.results && data.results.length > 0 ? (
          <table className={styles.table}>
            <thead className={styles.table_thead}>
              <tr>
                <th style={{ width: '80px' }} title="HTTP метод запроса">Метод</th>
                <th style={{ width: '300px' }} title="Путь запроса">Путь</th>
                <th style={{ width: '80px' }} title="HTTP статус ответа">Статус</th>
                <th title="Дата и время запроса">Дата запроса</th>
                <th title="IP адрес клиента">IP адрес</th>
                <th title="Email пользователя">Email</th>
                <th style={{ width: '250px' }} title="Django представление">Представление</th>
              </tr>
            </thead>
            <tbody className={styles.table_tbody}>
              {data.results.map((log, index) => (
                <tr key={log.id} onClick={() => handleRowClick(log)}>
                  ...
                  <td className={styles.viewCell} style={{ width: '250px' }}>{log.view}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {data && data.count > 0 && data.results && data.results.length > 0 && (
        <div className={styles.pagination}>
          ...
          <button 
            onClick={() => handlePageChange(filters.p - 1)} 
            disabled={!data.previous || filters.p <= 1} 
            className={styles.paginationButton}
          >
            Назад
          </button>
          <span className={styles.pageInfo}>
            Страница {filters.p} из {Math.ceil(data.count / filters.s)}{' '} 
            ({((filters.p - 1) * filters.s) + 1}-{Math.min(filters.p * filters.s, data.count)} из {data.count} записей)
          </span>
          <button 
            onClick={() => handlePageChange(filters.p + 1)} 
            disabled={!data.next} 
