...
export const MobileHeaderAdmin: FC = () => {
  ...
  const { data: currentTariffData, refetch: refetchTariff } = useFetchCurrentTariffPlanQuery(schoolData?.name as string, {
      ...
      <div className={styles.header_container}>
        ...
        <div className={`${styles.nav_links} ${menuOpen ? styles.hidden : ''}`}>
          {(schoolData?.extra_link ||
            ...
            schoolData?.vk_link) && (
            <div
              className={`${styles.nav_item} ${styles.social} ${isSocialOpen ? styles.active : ''}`}
          ...
          {userRole === RoleE.Admin && (
            <div
              className={`${styles.nav_item} ${styles.tariff} ${isTariffOpen ? styles.active : ''}`}
              onClick={toggleTariffMenu}
              ref={tariffMenuRef}
            >
              ...
              {isTariffOpen && currentTariffData && !('error' in currentTariffData) && (
                <div className={`${styles.submenu} ${styles.tariff}`}>
                  ...
                  <div className={styles.tariff_title}>
                    ...
                    <span>
                      {' '}
                      {currentTariffData?.tariff_name} {currentTariffData?.days_left} {currentTariffData?.days_left ? 'дней' : 'дней'}
                    </span>
                  </div>
                  ...
                  <div className={styles.tariff_text}>
                    Курсов:
                    <span>
                      {currentTariffData?.number_of_courses}/{currentTariffData?.tariff_details?.number_of_courses || '∞'}
                    </span>
                  </div>
                  ...
                  <div className={styles.tariff_text}>
                    Сотрудников:
                    <span>
                      {currentTariffData?.staff}/{currentTariffData?.tariff_details?.number_of_staff || '∞'}
                    </span>
                  </div>
                  ...
                  <div className={styles.tariff_text}>
                    Студентов:
                    <span>
                      {currentTariffData?.students}/{currentTariffData?.tariff_details?.total_students || '∞'}
                    </span>
                  </div>
                  ...
                  <div className={styles.tariff_text}>
                    Студентов в месяц:
                    <span>
                      {currentTariffData?.tariff_details?.student_count_by_month || 0}/{currentTariffData?.tariff_details?.students_per_month || '∞'}
                    </span>
                  </div>
                  <div className={styles.tariff_button} onClick={goToTariffPlans}>
                    Перейти на тариф
                    <img src={require('../../../Pages/TariffPlans/images/prizePersonal-tariff.png')} alt="Окно входа" />
                  </div>
                </div>
              )}
          ...
          <div
            className={`${styles.nav_item} ${styles.profile_item} ${isProfileOpen ? styles.active : ''}`}
          ...
          <div className={`${styles.nav_item} ${styles.exit_item} ${isExitActive ? styles.active : ''}`} onClick={handleExitClick}>
