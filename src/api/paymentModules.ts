...
const expressPayMainApi = createApi({
  ...
  endpoints: builder => ({
    fetchInvoiceDetails: builder.query<
      { Status: string; FirstName: string; Surname: string; Patronymic: string },
      { invoiceNo: number; apiKey: string }
    >({
...
export const { useLazyFetchInvoiceDetailsQuery, useCreateNewLinkMutation } = expressPayMainApi

export const { useLazyFetchInvoiceDetailsTestQuery, useCreateTestNewLinkMutation } = expressPayTestApi
