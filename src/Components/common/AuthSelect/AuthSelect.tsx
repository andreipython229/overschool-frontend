...
export const AuthSelect: FC<AuthSelectPropsT> = memo(({getInputVariant}) => {
    ...
    return (
        <div onClick={() => setContext(!context)} className={styles.container}>
            {variant === 'email'
                ? <svg className={styles.email} width="20" height="16" viewBox="0 0 20 16" fill="none"
                ...
                : <svg className={styles.phone} width="14" height="22" viewBox="0 0 14 22" fill="none"
                       xmlns="http://www.w3.org/2000/svg">
                    <path
            ...
            {context
                ? <div className={styles.popup}>
                    <div onClick={() => changeInputVariant('phone')}
                         className={variant === 'phone' ? styles.checked : ''}>
                        Номер телефона
                    </div>
                    <div onClick={() => changeInputVariant('email')}
                         className={variant === 'email' ? styles.checked : ''}>E-mail
                    </div>
                </div>
                : null}
